...
**不要提交 `models/` 目录。** 训练后的 `model.safetensors` 约 400MB，超过 Gitee 单文件 100MB 限制，会导致 `git push` 一直卡住或失败。

仓库已配置 `.gitignore` 忽略：

- `models/` — 本地运行 `python scripts/train.py` 生成
